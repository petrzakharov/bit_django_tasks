...
class Lead(models.Model):
    ...
    def make_state_in_progress(self): # TODO методы можно использовать во вьюшке для перевода состояния
...
Это полезно, если предполагается к примеру что статусы заказов могут менять в админке.

"""


# class Lead2(models.Model):
#     STATUS_FLOW = (
#         (LeadState.STATE_NEW, LeadState.STATE_IN_PROGRESS),
#         (LeadState.STATE_IN_PROGRESS, LeadState.STATE_POSTPONED),
#         (LeadState.STATE_IN_PROGRESS, LeadState.STATE_DONE),
...
#
#     def clean(self):
#         if (self._prev_state, self.state_id) not in Lead.STATUS_FLOW:
